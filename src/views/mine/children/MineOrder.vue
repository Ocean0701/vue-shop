<template>
  <div id="orderDetail">
    <!--导航栏-->
    <van-nav-bar title="我的订单" left-arrow :fixed="true" :border="true" @click-left="$router.go(-1)"></van-nav-bar>
    ...
    >
      <van-tab title="全部" name="a">
        <!-- <div class="shopCartListCon" v-for="(goods) in order" :key="goods.id">
          <div class="center">
            <img :src="goods.small_image" alt />
          </div>
          <div class="right">
            <a href="#">{{goods.product_name}}</a>
      ...
      <van-tab title="已出售" name="b">
          <div v-for="(product, index) in sell" :key="index" class="infoWrapper">
            <div class="imageWrapper">
                <div class="LazyLoad is-visible">
                    <img class="image" :src="product.small_image" alt=""></div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async fetch(){
        let arr = [0,1]
        let order = []
        let resultArr = await axios.all(arr.map(e => axios.get(`/api/cart/goods?type=${e}`)))
        resultArr.forEach(e => {
          if(e.data.errno === 0){
