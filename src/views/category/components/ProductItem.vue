<template>
    <div class="productWrapper">
        <div v-for="(product, index) in products" :key="index" class="infoWrapper">
            ...
            <div class="buyWrapper" v-if="tag">
                <van-button style="margin-left:4px; float:right;" color="#4bb0ff" size="small"  @click="showPopup(index)" >出售</van-button>
                    <van-popup  position="bottom" style="height:20rem;"  v-model="show">
                        
                        <comshop :products="productActive" v-if='show'>
                        </comshop>
                        
                    </van-popup>
                <van-button @click="addToCart(product)" style="margin-left:4px; float:right;" color="#57B609" size="small">购买</van-button>
            </div>
            <div class="buyWrapper" v-if="!tag">
                <van-button @click="del(product)" style="margin-left:4px; float:right;" type="danger" size="small">删除</van-button>
            </div>
        </div>
    </div>
</template>
...
<script>
    import comshop from './component/comshop'
...
import Axios from 'axios'
    ...
    export default {
        ...
        methods: {
            showPopup(index) {
                this.productActive = this.products[index]
                console.log(this.products[index])
                this.show=true
            },
            ...mapMutations(["ADD_GOODS"]),
            ...mapMutations(["ADD_SHOP"]),
            addToCart(goods){
                this.ADD_GOODS({
                    goodsId:goods.id,
                    ...
                    productName:goods.product_name,spec:goods.spec
                });
                //提示用户
                Toast({
                    message:'添加到购物车成功',
            ...
            del(goods){
                const that = this;
                Axios.get('/api/cart/del_shop?id='+goods.id).then(res=>{
